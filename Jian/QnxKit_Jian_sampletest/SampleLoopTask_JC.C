...
void SampleLoopTask::Task(){
	...
	while(!lost){
		...
		if(camera) { 
			...
			// This line is necessary not to have the MOTOR RUNS TOO FAST error when the hand is moved after this program runs and before running the control.
			handTheta_prev = -(HW->GetEncoderCount(IoHardware::ENC_0)) * ENC_RAD_PER_CNT / GR / 4; // just access the data previously read by GetEnc...()
		}
