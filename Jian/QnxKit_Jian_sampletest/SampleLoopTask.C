...
void SampleLoopTask::Task(){
    ...
    struct hostent *hp;
	//char msg[146]; char host[20];
	char buffer[15];
	const char * udpmsg = "dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd";
	const char * host = "192.168.1.114";
	unsigned int port = 13013;
	...
	if (bind(UDPsocket, (struct sockaddr *)&addr, sizeof(struct sockaddr))) printf("Bind failed.");
	if (fcntl(UDPsocket, F_SETFL, O_NONBLOCK) < 0) printf("Unblocking failed.");


	int runtime = 300;
	int runcnt = 0;

	int fd = creat("qnx_period.txt", S_IRUSR | S_IWUSR);

	while(!lost && runcnt <= runtime * SAMPLE_RATE){
		...
		if (abs(sec-0.004) < 1){
			...
			write(fd, buffer,sizeof(double));
			write(fd, "\n",sizeof("\n"));
		}

		if (sendto( UDPsocket, udpmsg, 104, 0, (struct sockaddr *)&addr, sizeof(addr))) 
