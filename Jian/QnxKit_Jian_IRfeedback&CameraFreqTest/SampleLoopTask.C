...
void SampleLoopTask::IRLEDdetect(){
	char currentIO, prevIO = 1;
	int n = 0;
	double ncycles, ncycles_prev, sec, cyc0, cyc1, total_time;

	cyc0 = ClockCycles();

	printf("\nIRLEDdetect thread on!!!\n");
	...
	while(n<10000){
		...
		HW->ProcessInput();
		currentIO = HW->ReadDigitalBit(IoHardware::FRAME_STATUS);
		//currentIO = HW->ReadDigitalBit(IoHardware::DIN_8);
		/*if (!currentIO)*/ 
		printf("IO:%d ", currentIO);
	...
	cyc1 = ClockCycles();
	total_time = (double)(cyc1 - cyc0)/cps;
	printf("\ntime used: %f sec. %d\n", total_time, IoHardware::FRAME_STATUS);
}
