...
ss = 100;
dist = zeros(n - 1-ss, num_markers);

for k = 1:num_markers
    for i = ss:n-1
        dist(i-ss+1,k) = sqrt((a(i,1,k)-a(i+1,1,k))^2 + (a(i,2,k)-a(i+1,2,k))^2);
    end
end
...
for i = 1:num_markers
    ...
    title(['Displacement between each point of marker# ' num2str(i) '(mm)']);
    ...
    xlabel('sample #');
    legend(['Displacement data #' num2str(i)], ['Average: ' num2str(mean(dist(:,i))) 'mm'], ['Max-Min: ' num2str(max(dist(:,i))-min(dist(:,i))) 'mm, std: ' num2str(std(dist(:,i))) 'mm']);
