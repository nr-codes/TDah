...
int = 1/num_markers;
for i = 1:num_markers
    plot(a(:,1,i),a(:,2,i),'linewidth',1,'color',[mod(i*0.3,1) mod((i-1)*0.6,1) mod((1-i*0.3),1)],'linestyle','o');
...
for i = 1:num_markers
    ...
    w_mean(i) = mean(w(:,i));
    w_std(i) = std(w(:,i));
    line([0 n], [w_mean(i) w_mean(i)],'linewidth',2,'color','r');
    w_max(i) = max(w(:,i));
    line([0 n], [w_max(i) w_max(i)],'linewidth',2,'color','g');
    w_min(i) = min(w(:,i));
    line([0 n], [w_min(i) w_min(i)],'linewidth',2,'color','g');
    xlabel('sample (sample rate = 250Hz)'); ylabel('in rad/sec');
end
w_mean
w_std
...
w_ave_std = std(w_ave)
line([0 n], [w_ave_mean w_ave_mean],'linewidth',2,'color','r');
w_ave_max = max(w_ave);
line([0 n], [w_ave_max w_ave_max],'linewidth',2,'color','g');
w_ave_min = min(w_ave);
line([0 n], [w_ave_min w_ave_min],'linewidth',2,'color','g');
xlabel('sample (sample rate = 250Hz)'); ylabel('in rad/sec');
...
% xlabel('sample (sample rate = 800Hz)'); ylabel('in rad/sec');
