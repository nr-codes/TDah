...
const double M_O = 0.142; //0.1503; //0.142; // mass of the object
const double RHO_H = 0.15; //0.15; // radius of the hand disk
const double RHO_HI = 0.0095; // inner radius of the hand
const double RHO_O = 0.081; //0.08// radius of the object disk
const double GRAV = 9.8 * sin(3.142592/180*77.6); //85.2 //81.9); //45.8); // gravitational acceleration
...
const double OBJ_X_OFFSET = 0.0909338 + 0.00224636 + (RHO_H+RHO_O)*0.00354; //0.00262; //*0.00301 //0.0848480; // in m
const double OBJ_Y_OFFSET = -0.0894877; //-0.0091731; // in m

const double TARGET_THETAOD = 1; // target velocity of the object. needs to be carefully determined before doing experiments.
const double TARGET_THETAHD = -4.0; //3.; //0.3; //-RHO_O/RHO_H*TARGET_THETAOD;
	...
	while(!lost){
		...
		if(camera) {// && !lost){
			...
			while(HW->ReadDigitalBit(IoHardware::FRAME_STATUS) == last_status){
				...
				if(++attempt == (int)(4.0e5/SAMPLE_RATE)) { // 3.0e5 must be found out by experiments to give smaller time to determine an error status fast
					...
					//strcpy(err_msg, "Frame not received -"); // does not work.
				}
			}
			//if (lost == 1)
			//{
			...
			// do not use this for thetahD control or doesn't matter because it's very small compared to thetah offset?
			...
			/// test, eta2 filter
			//eta2 = alpha*eta2 + (1-alpha)*eta2_prev;
