...
const double OBJ_X_OFFSET = 0.0848480;// in m
const double OBJ_Y_OFFSET = -0.0091731;  // in m


#define GAIN_C0			(0.6) //(-1.0) // -pi/2 < C0 < 0
#define GAIN_C1			(4.0) //(5.0) // 4.0
#define GAIN_C			(3.5) //(2.5)
#define GAIN_K			(3.5) //(2.5) 
	...
	*/
	...
	while(!lost){
		...
		if(camera) {// && !lost){
			...
			sum_h = 0; 
			for(s=0; s<1; s += ds){
				sum_h += COEFF_D * cos( alpha_eta1 + s*(eta2-alpha_eta1) ) * ds;
			}
			...
			DhDeta1 = 0; 
			for(s=0; s<1; s += ds){
				DhDeta1 += COEFF_D*sin( alpha_eta1 + s*(eta2-alpha_eta1) ) * (DalphaDeta1 * (s-1)) * ds;
			}
			...
			DhDeta2 = 0; 
			for(s=0; s<1; s += ds){
				DhDeta2 += -COEFF_D*sin( alpha_eta1 + s*(eta2-alpha_eta1) ) * s * ds;
			}
			...
			//test
			//vInp = alpha*vInp + (1-alpha)*vInp_prev;
			//vInp_prev = vInp;
