...
#define RHO_O			(0.08) // radius of the object disk
...
void SampleLoopTask::Task(){
	...
	while(!lost){
		...
		if(camera) {// && !lost){
			...
			//iCmd = (COEFF_H_NEG+COEFF_H_POS)/2 * vInp + (COEFF_J_NEG+COEFF_J_POS)/2 * sin(eta2) + (COEFF_MUD_NEG+COEFF_MUD_POS)/2 * handVel + (COEFF_FS_NEG+COEFF_FS_POS)/2 * SIGN_VEL; 
