...
const double GRAV = 9.8 * sin(3.142592/180*45.8); // gravitational acceleration
...
void SampleLoopTask::Task(){
	...
	while(!lost){
		...
		if(iCmd > (MAX_CURRENT_MA) ){ // 1.6 mA
			...
			HW->WriteDigitalBit(IoHardware::MOTOR_ENABLE, MOTOR_OFF);
			HW->motorStatus = MOTOR_OFF;
			printf("iCmd:%f, pCmd:%f, pos:%f, vCmd:%f, vel:%f, aCmd:%f feedforwardVal:%f\n", iCmd, pCmd, pos, vCmd, vel, aCmd, feedforwardVal);
