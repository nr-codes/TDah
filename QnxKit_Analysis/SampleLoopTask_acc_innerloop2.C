	...
	while(!lost){
		...
		*/
		...
		if(camera) {// && !lost){
			...
			handAcc = (handVel - handVel_prev)/sec;
			handAcc = alpha*handAcc + (1-alpha)*handAcc_prev;
			...
			//Test
			// desired acceleration
			//vInp = 30*DEG2RAD*(2*3.141592/2.0)*(2*3.141592/2.0)*sin(2*3.141592*(cnt)/SAMPLE_RATE/2.0); // period of the input: 1.0 sec
			vInp = 5*sin(2*3.141592*(cnt)/SAMPLE_RATE/2.0); // period of the input: 1.0 sec
			
			// adding corrective term
			//handVel = (handTheta - handTheta_prev) * SAMPLE_RATE;
			targetVel = handVel + vInp/SAMPLE_RATE;
			targetPos = handTheta + handVel/SAMPLE_RATE + .5*vInp/(SAMPLE_RATE*SAMPLE_RATE);
			correctedAcc = vInp + 0*(targetPos - handTheta) + 0*(targetVel - handVel);

			// for practical purpose
			double deciderVel = handVel;
			/*if ( abs(handVel) < 0.01 ) {
				deciderVel = 0;
			}*/
			...
			//if(deciderVel < 0) { 
			//if(targetVel < 0) { 
			if(correctedAcc < 0){ // acceleration.  Because the motor rotating direction was opposite when the modeling was done.
				//iCmd = COEFF_H_NEG * correctedAcc + COEFF_J_NEG * sin(eta2) + COEFF_MUD_NEG * handVel + COEFF_FS_NEG * SIGN_VEL; 
				//iCmd = (INERTIA*correctedAcc + DYN_FRIC_NEG*targetVel + STAT_FRIC_NEG*sign(targetVel))/KM_NEG;
				iCmd = (INERTIA*correctedAcc + DYN_FRIC_NEG*handVel + STAT_FRIC_NEG*sign(handVel))/KM_NEG;
			}
			else {
				//iCmd = COEFF_H_POS * correctedAcc + COEFF_J_POS * sin(eta2) + COEFF_MUD_POS * handVel + COEFF_FS_POS * SIGN_VEL; 
				//iCmd = (INERTIA*correctedAcc + DYN_FRIC_POS*targetVel + STAT_FRIC_POS*sign(targetVel))/KM_POS;
				iCmd = (INERTIA*correctedAcc + DYN_FRIC_POS*handVel + STAT_FRIC_POS*sign(handVel))/KM_POS;
			}
			//iCmd += 0.05*(targetVel - handVel) 
			handTheta_prev = handTheta;
			handVel_prev = handVel;
			handAcc_prev = handAcc;

			if(HW->motorStatus != MOTOR_ON) iCmd = 0;
		...
		num[1] = pCmd; //feedforwardVal; //pCmd; //eta2; //obj.theta; //vnum[0]; //obj.theta; //vel; //obj.theta;
		num[2] = vCmd; //feedbackVal; //vCmd; //obj.angVel; //obj.x; //vnum[1]; //obj.x1; //ampVCmd; //obj.x1; // position in m, *10 for cm
		//num[1] = feedforwardVal; //pCmd; //eta2; //obj.theta; //vnum[0]; //obj.theta; //vel; //obj.theta;
		//num[2] = feedbackVal; //vCmd; //obj.angVel; //obj.x; //vnum[1]; //obj.x1; //ampVCmd; //obj.x1; // position in m, *10 for cm
