...
const double GRAV = 9.8 * sin(3.142592/180*82.09); //85.2); //45.8); // gravitational acceleration
...
const double OBJ_X_OFFSET = 0.0928088; //0.0805625; //0.0848480; // in m
const double OBJ_Y_OFFSET = -0.0895314; //-0.009247; //-0.0091731; // in m
	...
	*/
	...
	while(!lost){
		...
		if(camera) {// && !lost){
			...
			vInp = -( GAIN_K1*(COEFF_sig2*eta1+COEFF_sig3*handVel) + GAIN_K2*eta2 + COEFF_sig1*COEFF_sig2*sin(eta2) )/COEFF_sig3;
