...
const double GRAV = 9.8 * sin(3.142592/180*85.2); // gravitational acceleration
...
#define GAIN_C0			(3.5) 
#define GAIN_C1			(5.0) 
#define GAIN_C2_FAR		(0.6) 
#define GAIN_C3			(1.9) 

/*#define GAIN_C		(6.0)
#define GAIN_C0			(3.0) 
#define GAIN_C1			(5.0) 
#define GAIN_C2_FAR		(0.2) 
#define GAIN_C3			(4.5) 
*/

//#define GAIN_C3_NEAR	(1.0)
//4jh#define GAIN_C2_NEAR	(0.2)
...
void SampleLoopTask::Task(){
	...
	while(!lost){
		...
		if(iCmd > (MAX_CURRENT_MA/1.6*1.4) ){ // 1.6 mA
			HW->SetAnalogOut(IoHardware::AMP_SIGNAL, 0.0); 
			HW->WriteDigitalBit(IoHardware::MOTOR_ENABLE, MOTOR_OFF);
			HW->motorStatus = MOTOR_OFF;
			...
			iCmd = MAX_CURRENT_MA/1.6*1.4;
		}
		else if(iCmd < (-MAX_CURRENT_MA/1.6*1.4) ){
			HW->SetAnalogOut(IoHardware::AMP_SIGNAL, 0.0); 
			HW->WriteDigitalBit(IoHardware::MOTOR_ENABLE, MOTOR_OFF);
			HW->motorStatus = MOTOR_OFF;
			printf("iCmd:%f, pCmd:%f, pos:%f, vCmd:%f, vel:%f, feedforwardVal:%f\n", iCmd, pCmd, pos, vCmd, vel, feedforwardVal);
