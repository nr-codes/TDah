...
void SampleLoopTask::Task(){
	...
	while(!lost){
		...
		if(camera) {// && !lost){
			...
			double GAIN_C2 = GAIN_C2_FAR; //NEAR; //FAR;  
			...
			*/
			...
			if (handVel > 0) {
				...
				iCmd = (COEFF_MUD_NEG*targetVel + COEFF_FS_NEG * SIGN_V) + 0.05*(targetVel - handVel) + 0.1*(0 - handAcc);
				
			}
			else {
				iCmd = (COEFF_MUD_POS*targetVel + COEFF_FS_POS * SIGN_V) + 0.05*(targetVel - handVel) + 0.1*(0 - handAcc);
