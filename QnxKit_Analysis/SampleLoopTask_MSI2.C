...
const double OBJ_X_OFFSET = 0.0786439; // in m. It may be better to get this value from constant speed control
...
void SampleLoopTask::Task(){
	...
	while(!lost){
		...
		if(camera) {// && !lost){
			...
			//eta2 = eta2 + 0.0013*sin(obj.theta + 9.305); 
			...
			u = -GAIN_K1*hDDD - GAIN_K2*hDD - GAIN_K3*hD - GAIN_K4*h;

			vInp = ( u - COEFF_sig2*COEFF_sig1*sin(eta2)*(-eta2D + COEFF_sig1*COEFF_sig2*cos(eta2)) )/(COEFF_sig1*COEFF_sig2*COEFF_sig3*cos(eta2)); 
			/*************************************************************/
		...
		num[1] = obj.angVel;
		*/
		num[0] = obj.theta; //correctedAcc; //eta1; //handVel; //pos; //handTheta; //pos; // angle in rad
		num[1] = obj.x; //feedforwardVal; //pCmd; //eta2; //obj.theta; //vnum[0]; //obj.theta; //vel; //obj.theta;
