	...
	while(!lost){
		...
		if(camera) {// && !lost){
			...
			*/
			...
			// for inner loop
			double alphaD = DalphaDeta1*(COEFF_D * sin(eta2)) + DalphaDeta2*(COEFF_A * eta1 - handVel/COEFF_B);
			handAcc = (handVel - handVel_prev)/sec;
			handAcc = alpha*handAcc + (1-alpha)*handAcc_prev;
			handAcc_prev = handAcc;

			// saturation function
			double SIGN_V = sign(targetVel);
			//if ( abs(targetVel) < 0.01 ) {
			//	SIGN_V = 100 * targetVel;  // 100 from 1/0.01
				//SIGN_V = 0;
			//}
			...
			if(handVel > 0){ // acceleration.  Because the motor rotaiting direction was opposite when the modeling was done.
				iCmd = COEFF_H_NEG * alphaD + COEFF_J_NEG * sin(eta2) + COEFF_MUD_NEG * targetVel + COEFF_FS_NEG * SIGN_V; 
			}
			else{
				iCmd = COEFF_H_POS * alphaD + COEFF_J_POS * sin(eta2) + COEFF_MUD_POS * targetVel + COEFF_FS_POS * SIGN_V; 
			}

			iCmd += 0.05*(targetVel - handVel) + 0.05*(alphaD - handAcc);
		...
		num[2] = targetVel; //vInp; //obj.angVel; //obj.x; //vnum[1]; //obj.x1; //ampVCmd; //obj.x1; // position in m, *10 for cm
